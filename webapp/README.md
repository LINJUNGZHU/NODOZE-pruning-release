...
最新：页面以用户 PDF 原文为主参考，显示确认代理与未判定节点；右侧图可切换预算剪枝和可展开到原始事件的证据聚合图。[完整实验与限制](../docs/pdf-groundtruth-study.md)。

“攻击节点识别”可选择 **多视角学习 · 实验**，点击“重新分析”。点击进程可查看属性新颖度、结构变化、事件意外程度及七维票数；这些百分位不是恶意概率。当前实测尚不如增强规则，所以默认继续使用增强规则。

结果下方的“从日志看发生了什么”按进程、文件、网络活动汇总，点击原始事件可以回查。下载报告包含共享上下文索引和完整引用；核验命令为 `python webapp/scripts/verify_attack_report.py 报告文件.json`。摘要由本地证据生成，未调用 LLM，也不宣称已确认所有 APT 阶段。

[三篇论文研究与实测报告](../docs/paper-fusion-study.md) · [原始对照结果](../docs/multiview-evaluation.json)
...
原页面“算法”选择“关系感知”或“完整 RASP-RCVP”；在自定义设置中可独立选择“渐进式证据剪枝”，支持 5%、10%、20%、30% 原始事件预算。点击重新分析后查看实际保留数、证书状态及逐边事件详情中的 RCVP 通道和删除审计。旧默认未切换，OPTC 仍属于开发诊断，相关性分值不是恶意概率。

配置、公式与 CLI 入口见 [实现说明](../docs/rasp-rcvp-method.md)，实验局限与对照见 [评估报告](../docs/rasp-rcvp-report.md)。无需 npm 构建；更新 Python 后重启现有后端进程以加载新代码。
