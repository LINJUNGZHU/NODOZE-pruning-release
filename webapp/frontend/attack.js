...
function renderAttack(){
 ...
 const activity=b?.activity_event_metrics;$('#activity-evaluation').textContent=activity?`候选进程活动：${fmt(activity.predicted_events)} 条事件；参考攻击事件找回 ${activity.matched_events} / ${activity.labeled_malicious_events}（${attackPercent(activity.recall)}），事件精确率 ${attackPercent(activity.precision)}。这是活动子图，不是完整攻击链准确率。`:'';
 const tapas=a.evaluation?.tapas_benchmark;$('#tapas-evaluation').textContent=tapas?`TAPAS 静态进程清单：找回 ${tapas.node_metrics.tp} / ${tapas.node_metrics.known_positive}（${attackPercent(tapas.node_metrics.recall)}），误报 ${tapas.node_metrics.fp}，漏报 ${tapas.node_metrics.fn}。与上方公开标注有 ${tapas.disagreement_with_published_process_ids.length} 个进程标签不同；TAPAS 原文件无时间标签，不能混用分母或当作当前时刻恶意证明。${(tapas.node_metrics.recall??0)<.9?'此口径尚未达到 90% 召回目标。':''}`:'本例没有匹配的 TAPAS 标注切片。';
...
function renderAttackNodes(){
 const a=state.data?.attack;if(!a)return;const q=$('#attack-search').value.trim().toLowerCase(),filter=$('#attack-filter').value,all=filter==='all',labels=a.evaluation?.published_benchmark?.labels||{};
