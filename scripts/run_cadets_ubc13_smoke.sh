...
run_id="${1:-smoke-$(date '+%Y-%m-%d_%H-%M-%S')}"
...
mkdir -p "${output_dir}"
printf '%s\n' "$$" > "${output_dir}/pid"
printf '{"status":"running","validation_status":"pending","run_id":"%s","pid":%s}\n' \
  "${run_id}" "$$" > "${status_file}"

cd "${project_dir}" || exit 1
python -u -m tc_pruning.cli poi-prefix-experiment \
  --db output/tc/cadets-e3-from-raw-2026-09-07_15-27-47.db \
  --spec configs/cadets_e3_poi_prefix_sweep_ubc13_smoke.json \
