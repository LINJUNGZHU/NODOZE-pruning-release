...
已整理 [2025—2026 社交网络传播论文与项目适配方案](docs/social-propagation-study.md)：优先考虑关系分离、反向源定位与正向证据核验、长链分组；新传播方案已完成离线适配实验，尚未替换默认算法。
...
flowchart TD
    L[OPTC 日志] --> C[完整候选时间窗]
    P[手动指定 POI 事件] --> H[统计严格早于 POI 的同主机历史]
    L --> H
    H --> R[交互频率与稀有度]
    C --> S[POI 传播与背景传播对比评分]
    P --> S
    ...
    C --> D[攻击进程推断：默认增强规则]
    D --> N[预测进程与可重放的证据路径]
    N --> G[活动保留、有限资源扩展、事件聚合]
    C --> G
    B --> E[独立评估]
    N --> E
    G --> E
    T[原始 PDF 整理并绑定日志 UUID 的真值] --> E
```

### 1. 输入与历史：不是把数据集砍一半

POI 是分析员手动选择的一条事件，包含源节点、目标节点、关系和时间。网页预设 POI 根据红队 PDF 选取，因此实验带有人工已知调查起点；报告另列排除 POI 进程端点后的结果。

候选图采用示例的完整固定时间窗，可能包含 POI 之后的日志，属于事后调查。**频率统计则查询同主机已建立索引的全部、严格早于 POI 的历史**，不使用等时刻或之后的事件，也不按候选数据量截取一半。

对于事件 `e`，先按交互语义生成签名，统计其历史次数 `c(e)`，计算 `rarity(e) = 1 / (1 + c(e))`。网络签名包括程序、关系、方向、远端 IP、目的端口和协议，去掉临时客户端源端口，避免每次连接都被误算成全新行为。未见过的交互稀有度同为 1，所以只用稀有度确实会产生大量相同分数，后续还需要图结构对比。

### 2. 边评分：罕见，而且与 POI 相比背景更相关

在去重的“源节点—目标节点—关系”交互图上，分别计算以 POI 端点为起点的个性化 PageRank（PPR）和以进程节点为背景起点的 PPR。重复交互在传播时共用一个通道，原始事件仍保留用于展示与核验。

设节点的两种传播值为 `p(v)`、`b(v)`，对比提升为 `lift(v) = log(1 + max(p(v) / b(v) - 1, 0))`。边分数由两端提升的几何平均乘以历史稀有度权重得到，再归一化；实现对背景零值作数值保护，并将 POI 分数设为 1。配置还允许混入少量未对比的传播分数；证据模式使用纯对比得分。

直观上，常见枢纽虽然连接很多节点，但如果对 POI 没有额外关联，其分数会受到抑制。**这个分数表示调查相关性，不是攻击概率，也不保证每条边分数都不同。**

### 3. 剪枝：不仅看单条边是否超过阈值

先寻找与 POI 相连的时序路径和共同祖先分叉，再按完整证据组选择边。路径必须能够在原始有向日志中重放，事件时间严格递增；同一时间戳不能冒充先后因果。

- `evidence`：要求对比提升通过数值误差界核验，并具有有效时序见证；综合语义交互族的覆盖收益与新增边成本，贪心选择完整证据组。
- `context`：放宽得分资格以保留更多调查上下文，仍按时序证据组、交互多样性和预算选择。

预算 `B = max(1, floor(候选边数 × 保留比例))` 是上限，不要求填满。完整证据组一起保留，共享边只计一次成本，因此低分连接边也可能留下。审计可以解释每条边为什么留下或被删除。

数值误差界只保证“算出的正提升不是迭代误差造成的”；时序核验只保证日志中的方向和先后关系成立。它们都不是恶意判定正确率、统计误报率或全局最优解的证明。

### 4. 攻击节点：增强规则与受约束的进程链

默认检测器 `rules` 在**剪枝前的完整候选日志**上运行，避免先剪掉证据再做检测。当前主要面向 PowerShell 攻击行为：

1. 找到同时具有编码命令和隐藏窗口参数的 PowerShell/pwsh 创建事件，并要求进程结束前出现后续出站通信或子进程创建作为佐证。
2. 有条件地纳入直接启动它的解释器父进程：必须观察到父进程自身创建，且启动间隔不超过 120 秒，不任意向上标记所有祖先。
3. 沿实际 `PROCESS_CREATE` 边纳入后代，最多 4 代、根进程创建后 600 秒内；遵守严格时间顺序和进程终止边界，按 UUID 区分身份。

每个预测附带根行为证据、进程创建链及原始事件 ID，可由验证器重放。单纯异常高分只进入待复核列表，共享文件或相同 PID 不足以判为攻击。后代推断表示可能参与攻击活动，不等于证明该程序文件本身是恶意软件。文件和网络端点作为资源单列。

输出路径也必须对应真实有向时序事件；共享文件形成的路径仅说明依赖联系。活动摘要由日志确定性生成，当前没有调用 LLM，也不会补写日志中不存在的攻击成功情节。

### 5. 证据图：保留判定依据，再压缩展示

证据图与上面的预算剪枝结果分别输出，不改写原剪枝保留标记，也不改变攻击节点预测。它先保留全部判定证据、已输出路径、预测进程的活动，以及学习模型使用的上下文引用。

随后只做一轮文件扩展：从预测进程的写入/创建，追踪严格更晚且 120 秒内的同文件读取；文件连接至少 5 个非预测进程时停止这种扩展。新增读取者只属于上下文，不因此升级为攻击节点。

最后按“源 UUID、目标 UUID、关系、1 分钟时间块”聚合事件，每组保留全部原始 ID、数量及起止时间。**可逆性仅针对所选证据子图**，不意味着删掉的整个原图可以恢复，也不意味着聚合组本身就是一条攻击路径。

### 6. 深度学习目前处于什么位置

`neural` 与 `multiview` 是可切换的实验检测器，分别探索自监督图学习和属性、结构、因果多视角联合判断；它们没有与默认规则自动叠加。当前实测不足以支持替换规则。论文提供设计参考，本项目不声称完整复现 OCR-APT、Slot 或 PROVFUSION。

[增强规则与边界](docs/rule-lineage-method.md) · [神经模型实验](docs/deep-graph-method.md) · [三篇论文适配与多视角实验](docs/paper-fusion-study.md) · [后续十五篇论文的实际阅读范围与取舍](docs/pdf-groundtruth-study.md#十五篇论文的阅读结果与取舍)

### 7. Ground Truth 与当前效果

OPTC 默认真值来自 `OpTCRedTeamGroundTruth.pdf`，按页码和原文记录整理，再通过主机、日期、PID、时间及 C2 通信绑定实际日志 UUID；不是直接采用 TAPAS 节点名单。除上述手动 POI 先验外，真值不进入打分、规则推断或证据扩展。新上传压缩包中的 DARPA E3/E5 标签另作对应数据集评估。

当前规则找回 PDF 在这些窗口内明确确认的 **2/2 个代理**，另有 **7 个预测进程未被原文判定**。未知节点不当作正常负例，因此目前不能给出完整 Accuracy、Precision 或 F1，也不能把 2/2 宣称为全体攻击节点召回 100%。

| OPTC 攻击窗口 | 原始事件数 | 预算剪枝的 PDF 参考事件保留率 | 证据图事件数 / 展示聚合组数 | 证据图的 PDF 参考事件保留率 |
|---|---:|---:|---:|---:|
| 默认窗口 | 27,011 | 65.83% | 744 / 606 | 265/278（95.32%） |
| 30 分钟 | 86,318 | 67.48% | 1,551 / 1,397 | 934/947（98.63%） |
| 60 分钟 | 188,609 | 67.10% | 2,584 / 2,430 | 1,966/1,979（99.34%） |

这些是同主机、互相重叠的开发窗口；默认窗口剪枝为 `evidence`，另两个为 `context`，证据图不受同一边预算约束，不能据此宣称同预算算法优越。PDF 参考事件还含命名文件的正常读取，因此上表衡量证据保留，不是完整攻击事件或攻击路径召回。三个窗口均有 13 条参考事件未保留，完整解释及可复算数据见 [最新评估报告](docs/pdf-groundtruth-study.md#实验解释)。

### 8. 对应代码

| 环节 | 实现入口 |
|---|---|
| OPTC 主流程、历史频率与模式切换 | [optc_investigation.py](tc_pruning/optc_investigation.py) |
| 对比传播评分 | [rasp.py](tc_pruning/rasp.py) |
| 攻击推断与增强进程链 | [attack_inference.py](tc_pruning/attack_inference.py)、[rule_lineage.py](tc_pruning/rule_lineage.py) |
| 证据选择、资源扩展与聚合 | [context_graph.py](tc_pruning/context_graph.py) |
| PDF 身份解析与独立评估 | [pdf_groundtruth.py](tc_pruning/pdf_groundtruth.py)、[attack_evaluation.py](tc_pruning/attack_evaluation.py) |
