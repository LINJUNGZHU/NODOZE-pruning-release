> **三篇论文适配实验**：已研读 OCR-APT、Slot、PROVFUSION 并实现独立多视角学习、共享证据上下文和可核验活动摘要。新学习模型默认窗口命中 3/8、误报 11，尚不如增强规则的 8/8、误报 1，因此保留为实验选项。[研究、取舍与实测](docs/paper-fusion-study.md)。

本轮更新：[PDF 原文 Ground Truth、上传压缩包复算与证据聚合图](docs/pdf-groundtruth-study.md)。默认评估不再使用 TAPAS 名单；未判定节点不当作正常。
